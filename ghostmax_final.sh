...
public class CryptoHelper {
    ...
    public static SharedPreferences getEncryptedPrefs(Context ctx) throws GeneralSecurityException {
...
if [ -d ".git" ]; then
    git add -A
    git commit -m "Finales Update: Redesign, Geheim-Modus, Bild-Generierung, Bugfixes (v${NEW_VERSION})" || echo "ℹ️ Nichts zu committen."
    git push || echo "⚠️ git push fehlgeschlagen - bitte Remote/Branch pruefen."
else
    echo "ℹ️ Kein .git-Ordner gefunden - Git-Schritt uebersprungen. Falls gewuenscht, 'git init' und Remote selbst einrichten."
fi

echo "============================================================"
echo "✅ Fertig. APK: $APK_DEST"
echo "✅ GitHub-Workflow liegt unter .github/workflows/build.yml - baut bei jedem 'git push' automatisch in der Cloud mit."
echo "============================================================"
